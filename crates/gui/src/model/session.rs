...
#[cfg(feature = "desktop")]
use crate::agent::acp::{self, Event, Launch, Reply, Session};
#[cfg(feature = "desktop")]
...
#[cfg(feature = "desktop")]
use artifact::session::chat::ToolStatus;
...
impl ChatSession {
    ...
    ///
    /// Nothing without the `desktop` feature: there is no agent to point at.
    ...
    pub fn live(&self) -> bool {
        #[cfg(feature = "desktop")]
        return matches!(self.connection, Connection::Live(_));
        #[cfg(not(feature = "desktop"))]
        false
    }
    ...
    pub fn set_config(&mut self, config_id: &str, value: SessionConfigOptionValue) {
        ...
        #[cfg(feature = "desktop")]
        {
            ...
            if !self.config.iter().any(|option| {
                &*option.id == config_id && session_preferences::supports(option, &value)
            }) {
                return;
            }
            ...
            let saved = session_preferences::remember_config(
                &self.cwd,
                &self.entry,
                config_id,
                value.clone(),
