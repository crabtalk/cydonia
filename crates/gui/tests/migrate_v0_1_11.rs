...
use cydonia_gui::model::migrate::v0_1_11::rename_keys;
...
fn the_panels_a_session_held_are_dropped_and_the_width_is_kept() {
    assert_eq!(
        cydonia_gui::model::migrate::v0_1_11::legacy_width(LEGACY_PANELS),
        Some(Some(380.0))
    );
}
...
fn a_file_already_carried_is_left_alone() {
    assert_eq!(
        cydonia_gui::model::migrate::v0_1_11::legacy_width(CURRENT_PANELS),
        None,
        "a second pass must not throw away a panel written since"
    );
    assert_eq!(
        cydonia_gui::model::migrate::v0_1_11::legacy_width(r#"{"projects":{}}"#),
        None,
