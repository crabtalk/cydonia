...
use crate::{model::workspace::Workspace, view::settings::SettingsWindow};
use bezel::{
...
impl SettingsWindow {
    pub(super) fn mcp_body(&self, cx: &Context<Self>) -> AnyElement {
        ...
        theme
            .group_box()
            .children(Switch::ALL.map(|switch| self.mcp_switch(switch, cx)))
            .children(quiet.then(|| {
    ...
    /// kind of thing.
    fn mcp_switch(&self, switch: Switch, cx: &Context<Self>) -> AnyElement {
        let theme = Theme::of(cx).clone();
        let on = switch.on(self.workspace.read(cx));
        let (glyph, title, blurb) = switch.copy();
        theme
            .card_row(switch == Switch::Serve)
            .child(
                div()
                    .flex_none()
                    .size(px(18.))
            ...
            .child(
                div()
                    .id(("mcp-switch", switch as usize))
                    .cursor_pointer()
                    .child(theme.toggle(on))
                    .on_click(cx.listener(move |this, _, _, cx| {
                        this.workspace.update(cx, |workspace, cx| match switch {
                            Switch::Serve => workspace.set_mcp_serve(!on, cx),
                            Switch::Write => workspace.set_mcp_write(!on, cx),
                        });
                        cx.notify();
                    })),
            )
