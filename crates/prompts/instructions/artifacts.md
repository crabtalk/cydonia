...
filesystem access may create an article's own assets/ directory and read or
write media files there. Use unique filenames and preserve existing assets
unless replacement or removal is requested. Article read and creation results
include assets_path, an absolute path on the Cydonia host. This exception does not override read-only
