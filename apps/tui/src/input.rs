...
fn collect_candidates(line: &str, pos: usize, extra_commands: &[String]) -> Vec<String> {
    let prefix = &line[..pos];
    ...
    let extra_prefix = &typed[1..];
    for name in extra_commands {
        if name.starts_with(extra_prefix) {
...
impl DropdownState {
    ...
    /// Window of candidates to show. Clamped to the list first — typing
    /// can shrink the candidates while `scroll` still points past the end.
    fn visible_range(&self) -> std::ops::Range<usize> {
        let len = self.candidates.len();
        if len == 0 {
            return 0..0;
        }
        let vis = MAX_DROPDOWN_VISIBLE.min(len);
        let mut start = self.scroll.min(len - vis);
        if self.selected < start {
            start = self.selected;
        } else if self.selected >= start + vis {
            start = self.selected + 1 - vis;
        }
        start..start + vis
    }
...
impl InputState {
    pub fn new(history: History, extra_commands: Vec<String>) -> Self {
        Self {
            buf: InputBuffer::new(),
            history,
            dropdown: None,
